...
#region TCPHandler class
	...
	struct deviceRegisters
	{
		...
		public bool[] discreteInputs; // = new Int16[65535];
	}
 ...
 //       public Int16[][] holdingRegisters = new Int16[numberOfDevices][];
 //       public Int16[][] inputRegisters = new Int16[numberOfDevices][];
 //       public Int16[][] coils = new Int16[numberOfDevices][];
 //       public Int16[][] discreteInputs = new Int16[numberOfDevices][];

		deviceRegisters[] regDevices = new deviceRegisters[numberOfDevices];
...
//        private byte unitIdentifier = 1;
        ...
        #region events
        public delegate void CoilsChanged(int coil, int numberOfCoils, byte unitIdentifier);
        public event CoilsChanged coilsChanged;

        public delegate void HoldingRegistersChanged(int register, int numberOfRegisters, byte unitIdentifier);
		...
		public ModbusServer()
		{
			...
			{
				regDevices[i].holdingRegisters = new Int16[65535];
				regDevices[i].inputRegisters = new Int16[65535];
				regDevices[i].coils = new bool[65535];
				regDevices[i].discreteInputs = new bool[65535];
			}
		}
...
//                    return;
                ...
                this.CreateAnswer(receiveDataThread, sendDataThread, stream,portIn, ipAddressIn, receiveDataThread.unitIdentifier);
                //this.sendAnswer();
        ...
        private void CreateAnswer(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            ...
            switch (receiveData.functionCode)
            {
                ...
                case 1:
                    if (!FunctionCode1Disabled)
                        this.ReadCoils(receiveData, sendData, stream, portIn, ipAddressIn, unitIdentifier);
                    else
                    {
                ...
                case 2:
                    if (!FunctionCode2Disabled)
                        this.ReadDiscreteInputs(receiveData, sendData, stream, portIn, ipAddressIn, unitIdentifier);
                    else
                    {
                        sendData.errorCode = (byte)(receiveData.functionCode + 0x80);
                        sendData.exceptionCode = 1;
                ...
                case 3:
                    if (!FunctionCode3Disabled)
                        this.ReadHoldingRegisters(receiveData, sendData, stream, portIn, ipAddressIn, unitIdentifier);
                    else
                    {
                        sendData.errorCode = (byte)(receiveData.functionCode + 0x80);
                        sendData.exceptionCode = 1;
                ...
                case 4:
                    if (!FunctionCode4Disabled)
                        this.ReadInputRegisters(receiveData, sendData, stream, portIn, ipAddressIn, unitIdentifier);
                    else
                    {
                        sendData.errorCode = (byte)(receiveData.functionCode + 0x80);
                        sendData.exceptionCode = 1;
                ...
                case 5:
                    if (!FunctionCode5Disabled)
                        this.WriteSingleCoil(receiveData, sendData, stream, portIn, ipAddressIn, unitIdentifier);
                    else
                    {
                        sendData.errorCode = (byte)(receiveData.functionCode + 0x80);
                        sendData.exceptionCode = 1;
                ...
                case 6:
                    if (!FunctionCode6Disabled)
                        this.WriteSingleRegister(receiveData, sendData, stream, portIn, ipAddressIn, unitIdentifier);
                    else
                    {
                        sendData.errorCode = (byte)(receiveData.functionCode + 0x80);
                        sendData.exceptionCode = 1;
                ...
                case 15:
                        if (!FunctionCode15Disabled)
                            this.WriteMultipleCoils(receiveData, sendData, stream, portIn, ipAddressIn, unitIdentifier);
                        else
                        {
                            sendData.errorCode = (byte)(receiveData.functionCode + 0x80);
                            sendData.exceptionCode = 1;
                ...
                case 16:
                        if (!FunctionCode16Disabled)
                            this.WriteMultipleRegisters(receiveData, sendData, stream, portIn, ipAddressIn, unitIdentifier);
                        else
                        {
                            sendData.errorCode = (byte)(receiveData.functionCode + 0x80);
                            sendData.exceptionCode = 1;
                ...
                case 23:
                        if (!FunctionCode23Disabled)
                            this.ReadWriteMultipleRegisters(receiveData, sendData, stream, portIn, ipAddressIn, unitIdentifier);
                        else
                        {
                            sendData.errorCode = (byte)(receiveData.functionCode + 0x80);
                            sendData.exceptionCode = 1;
        ...
        private void ReadCoils(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            ...
            if (sendData.exceptionCode == 0)
            {
                ...
                Array.Copy(regDevices[unitIdentifier].coils, receiveData.startingAdress + 1, sendData.sendCoilValues, 0, receiveData.quantity);
            }
            if (true)
            {
                Byte[] data;
        ...
        private void ReadDiscreteInputs(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            sendData.response = true;
            ...
            if (sendData.exceptionCode == 0)
            {
                ...
                sendData.sendCoilValues = new bool[receiveData.quantity];
                Array.Copy(regDevices[unitIdentifier].discreteInputs, receiveData.startingAdress + 1, sendData.sendCoilValues, 0, receiveData.quantity);
            }
            if (true)
            {
                Byte[] data;
        ...
        private void ReadHoldingRegisters(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            sendData.response = true;
            ...
            if (sendData.exceptionCode == 0)
            {
                ...
                Buffer.BlockCopy(regDevices[unitIdentifier].holdingRegisters, receiveData.startingAdress * 2 + 2, sendData.sendRegisterValues, 0, receiveData.quantity * 2);
            }
                if (sendData.exceptionCode > 0)
        ...
        private void ReadInputRegisters(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            sendData.response = true;
            ...
            if (sendData.exceptionCode == 0)
            {
                sendData.byteCount = (byte)(2 * receiveData.quantity);
                sendData.sendRegisterValues = new Int16[receiveData.quantity];
                Buffer.BlockCopy(regDevices[unitIdentifier].inputRegisters, receiveData.startingAdress * 2 + 2, sendData.sendRegisterValues, 0, receiveData.quantity * 2);
            }
                if (sendData.exceptionCode > 0)
                    sendData.length = 0x03;
                else
        ...
        private void WriteSingleCoil(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            sendData.response = true;
            ...
            if (sendData.exceptionCode == 0)
            {
                ...
                {
                    regDevices[unitIdentifier].coils[receiveData.startingAdress + 1] = true;
                }
                if (receiveData.receiveCoilValues[0] == 0x0000)
                {
                    regDevices[unitIdentifier].coils[receiveData.startingAdress + 1] = false;
                }
            }
                if (sendData.exceptionCode > 0)
                    sendData.length = 0x03;
            ...
            if (true)
            {
                ...
                if (coilsChanged != null)
                    coilsChanged(receiveData.startingAdress, 1, unitIdentifier);
            }
        }

        private void WriteSingleRegister(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            sendData.response = true;
            ...
            if (sendData.exceptionCode == 0)
            {
                regDevices[unitIdentifier].holdingRegisters[receiveData.startingAdress + 1] = unchecked((short)receiveData.receiveRegisterValues[0]);
            }
                if (sendData.exceptionCode > 0)
                    sendData.length = 0x03;
                else
            ...
            if (true)
            {
                ...
                if (holdingRegistersChanged != null)
                    holdingRegistersChanged(receiveData.startingAdress, 1 ,unitIdentifier);
            }
        }

        private void WriteMultipleCoils(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            sendData.response = true;
            ...
            if (sendData.exceptionCode == 0)
            {
                for (int i = 0; i < receiveData.quantity; i++)
                {
                    ...
                    if ((receiveData.receiveCoilValues[i / 16] & (ushort)mask) == 0)
                        regDevices[unitIdentifier].coils[receiveData.startingAdress + i + 1] = false;
                    else
                        regDevices[unitIdentifier].coils[receiveData.startingAdress + i + 1] = true;
        ...
        private void WriteMultipleRegisters(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            sendData.response = true;
            ...
            if (sendData.exceptionCode == 0)
            {
                for (int i = 0; i < receiveData.quantity; i++)
                {
                    regDevices[unitIdentifier].holdingRegisters[receiveData.startingAdress + i + 1] = unchecked((short)receiveData.receiveRegisterValues[i]);
                }
            }
            if (sendData.exceptionCode > 0)
                sendData.length = 0x03;
            ...
            if (true)
            {
                ...
                if (holdingRegistersChanged != null)
                    holdingRegistersChanged(receiveData.startingAdress, receiveData.quantity, unitIdentifier);
            }
        }

        private void ReadWriteMultipleRegisters(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn, byte unitIdentifier)
        {
            sendData.response = true;
            ...
            if (sendData.exceptionCode == 0)
            {
                ...
                Buffer.BlockCopy(regDevices[unitIdentifier].holdingRegisters, receiveData.startingAddressRead * 2 + 2, sendData.sendRegisterValues, 0, receiveData.quantityRead * 2);

				for (int i = 0; i < receiveData.quantityWrite; i++)
				{
					regDevices[unitIdentifier].holdingRegisters[receiveData.startingAddressWrite + i + 1] = unchecked((short)receiveData.receiveRegisterValues[i]);
                }
            ...
            if (true)
            {
                ...
                if (holdingRegistersChanged != null)
                    holdingRegistersChanged(receiveData.startingAddressWrite, receiveData.quantityWrite, unitIdentifier);
            }
        }
